...
export default function Nav() {
  ...
  if (role === "admin") {
    return (
      <nav className="fixed bottom-0 left-0 right-0 z-50 flex items-center justify-around px-6 py-3 bg-[#FCF8F5]/90 backdrop-blur-md border-t border-[#EBE3D5] shadow-lg">
      ...
      <div className={`relative w-full flex items-center justify-between px-2 pb-6 pt-3 pointer-events-auto border-t border-transparent ${!isNewMemory ? "bg-[#FCF5F0]" : ""}`} style={{ filter: !isNewMemory ? "drop-shadow(0 -4px 10px rgba(0,0,0,0.04))" : "none" }}>
        ...
        <div className="flex justify-around items-center w-full h-full z-10 px-2">
          ...
          <NavItem href="/dashboard" icon={
            <svg width="24" height="24" viewBox="0 0 24 24" fill={pathname === "/dashboard" ? "#B97C7C" : "none"} stroke={pathname === "/dashboard" ? "#B97C7C" : "#8B6A5B"} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
          ...
          <NavItem href="/gallery" icon={
            <svg width="24" height="24" viewBox="0 0 24 24" fill={pathname === "/gallery" ? "#B97C7C" : "none"} stroke={pathname === "/gallery" ? "#B97C7C" : "#8B6A5B"} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
          ...
          <div className="relative flex justify-center w-[70px] -mt-10 z-20">
            <Link
              ...
              style={{ backgroundColor: "#C28C8C", border: "2px dashed #FCF5F0", boxShadow: "0 4px 12px rgba(185, 124, 124, 0.4)" }}
            >
          ...
          <NavItem href="/profile" icon={
            <svg width="24" height="24" viewBox="0 0 24 24" fill={pathname === "/profile" ? "#B97C7C" : "none"} stroke={pathname === "/profile" ? "#B97C7C" : "#8B6A5B"} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
          ...
          >
            ...
            <span className="text-[14px] font-semibold text-[#8B6A5B]" style={{ fontFamily: "var(--font-caveat), cursive" }}>Logout</span>
          </button>
        </div>
      </div>
    </nav>
...
function NavItem({
  ...
  return (
    ...
    >
      {icon}
      <span className={`text-[14px] transition-colors ${active ? "text-[#B97C7C] font-bold" : "text-[#8B6A5B]"}`} style={{ fontFamily: "var(--font-caveat), cursive" }}>
        {label}
      </span>
    </Link>
  );
